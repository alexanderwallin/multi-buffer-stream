 ...
 * Constructor
 */
let MultiBufferStream = function(buffer) {
...
MultiBufferStream.prototype.insertBuffer = function(ab) {
  ...
  for (let i = 0; i < this.buffers.length; i++) {
    ...
    if (ab.fileStart <= b.fileStart) {
      ...
      let offset = b.fileStart + b.byteLength - ab.fileStart;
      let newLength = ab.byteLength - offset;
      if (newLength > 0) {
...
MultiBufferStream.prototype.logBufferLevel = function(info) {
  ...
  let log = (info ? Log.info : Log.debug)
...
MultiBufferStream.prototype.mergeNextBuffer = function() {
  ...
  if (this.bufferIndex+1 < this.buffers.length) {
    ...
    if (next_buffer.fileStart === this.buffer.fileStart + this.buffer.byteLength) {
      let oldLength = this.buffer.byteLength;
      let oldUsedBytes = this.buffer.usedBytes;
      let oldFileStart = this.buffer.fileStart;
...
MultiBufferStream.prototype.getEndFilePositionAfter = function(pos) {
  let index = this.findPosition(true, pos, false);
  if (index !== -1) {
